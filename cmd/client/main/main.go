...
const volumeInfoFormat = `{{with .GetHandle}}{{$name := .GetId}}` +
	`{{printf "%s\t" $name}}{{end}}{{"\n"}}`
...
func listVolumes(
	...
	go func() {
		tok := startingToken
		for {
			vols, next, err := utils.ListVolumes(
				ctx,
				client,
				version,
				maxEntries,
...
func getNodeID(
	...
	// execute the rpc
	if nodeID, err = utils.GetNodeID(ctx, client, version); err != nil {
		return err
	}
